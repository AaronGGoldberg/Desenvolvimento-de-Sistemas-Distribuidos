...
@@ -225,103 +225,264 @@ Este projeto tem como finalidade demonstrar na prática:
* Arquitetura de microsserviços
* Uso de API Gateway
* Integração entre serviços
* Implementação de HATEOAS
...
### Objetivo do serviço SOAP
A ideia desta parte da atividade é mostrar o mesmo tema do sistema de contas online, mas usando SOAP em vez de REST/JSON. Aqui a comunicação principal é feita com XML, Envelope SOAP, Body, namespaces e um contrato WSDL.

O serviço SOAP mantém o mesmo tema do sistema de contas online e expõe operações síncronas usando XML, Envelope SOAP, Body, namespaces e contrato WSDL.
### O que foi implementado
...
1. `consultarConta`
   * Entrada: `contaId`
   * Saída: `contaId`, `nome`, `email`, `saldo`, `status`
* se você abrir pelo `localhost`, o WSDL aponta para `http://localhost:3003/soap`;
* se você abrir pela URL pública do Codespaces, o WSDL aponta para `https://<seu-codespace>-3003.app.github.dev/soap`.

2. `simularOperacao`
   * Entrada: `contaId`, `tipo` (`deposito` ou `saque`) e `valor`
   * Saída: `contaId`, `tipo`, `valor`, `saldoAntes`, `saldoDepois`, `mensagem`
Isso foi feito para facilitar o uso no SoapUI, evitando ter que trocar manualmente o endpoint depois de importar o WSDL.

Em caso de entrada inválida, conta inexistente ou saque sem saldo suficiente, o servidor retorna um `soapenv:Fault`.
### Pré-requisitos
...
```bash
cd SistemaContasOnline/soap-service
node server.js
```

Se você já estiver dentro da pasta `SistemaContasOnline`, pode usar:
...
```bash
cd SistemaContasOnline/soap-client
python3 client.py
```
...
O cliente Python consome o servidor Node usando requisições SOAP XML, demonstrando interoperabilidade entre linguagens diferentes.
O cliente Python executa três chamadas:
